...
<div class="col-lg-6">
  ...
  <div class="card">
    <div class="card-body">
      ...
      <form class="row g-3" method="post" action="{{ route('candidate.store',$candidate->id) }}">
        @csrf
        ...
        <div class="col-md-12">
          ...
          <select id="inputState" name="location_id" class="form-select">
            ...
            <option value="{{ $location->id }}" {{ ($candidate->location_id == $location->id) ? 'selected' : ''  }}>{{ $location->name }}</option>
            @endforeach
          </select>
        </div>
